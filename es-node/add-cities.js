...
for (state in states) {
    for (city in states[state]) {
        var cityName = states[state][city];
        var cityClean = removeSpecial(cityName);
        var stateClean = removeSpecial(state);
        var cityId = stateClean + '-' + cityClean;
        cityId = cityId.trim();
        ...
        var dataRecord = {
            ...
            'fixedStateName': state,
            'cityName': cityName = states[state][city]
        };
